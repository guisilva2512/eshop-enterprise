using Dapper;
using eShopEnterprise.Pedidos.API.Application.DTO;
...
namespace eShopEnterprise.Pedidos.API.Application.Queries
{
    ...
    public class PedidoQueries : IPedidoQueries
    {
        ...
        public async Task<PedidoDTO> ObterUltimoPedido(Guid clienteId)
        {
            ...
            // Multi query
            // Não vai dar certo, porque endereço não é uma tabela fora
            //var pedido = await _repository.ObterConexao()
            ...
            var pedido = await _repository.ObterConexao().QueryAsync<dynamic>(sql, new { clienteId });
        ...
        public async Task<PedidoDTO> ObterPedidosAutorizados()
        {
            ...
            await _repository.ObterConexao().QueryAsync<PedidoDTO, PedidoItemDTO, PedidoDTO>(sql,
                (p, pi) =>
                {
